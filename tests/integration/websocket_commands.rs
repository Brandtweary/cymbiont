use crate::common::test_harness::{
    assert_phase, authenticate_websocket, connect_websocket, expect_success,
    get_single_open_graph_id, read_auth_token, send_command, setup_with_graph, PostShutdown,
    PreShutdown, WsConnection,
};
...
fn test_multiple_block_updates(
    ws: &mut WsConnection,
    ...
    validator.expect_create_block(
        &parent_id,
        "Parent block for update test",
        Some("test-update-page"),
        Some(graph_id),
    );
    ...
    validator.expect_update_block(
        &child_id,
        "Final update - this should be the final content",
        Some(graph_id),
    );
...
fn test_graph_operations(
    ...
    let graph_ids: Vec<String> = graphs
        ...
        .collect();
    assert!(
        graph_ids.contains(&original_graph_id),
        "Original graph should be in list"
    );
    assert!(
        graph_ids.contains(&new_graph_id),
        "New graph should be in list"
    );

    // Verify the new graph has correct metadata
    let new_graph = graphs
        .iter()
...
fn test_error_cases(ws: &mut WsConnection, port: u16, _original_graph_id: &str) {
    ...
    assert!(
        !authenticate_websocket(&mut invalid_auth_ws, "invalid-token"),
...
fn validate_registry_state(data_dir: &std::path::Path, original_graph_id: &str) {
    ...
    assert!(
        open_graphs
            .iter()
            .any(|g| g.as_str() == Some(original_graph_id)),
...
pub fn test_websocket_commands() {
    ...
    let result = std::panic::catch_unwind(move || {
        ...
        validator.expect_create_block(
            &new_block_id,
            "WebSocket test block with **bold** and *italic* text",
            Some("test-websocket-page"),
        ...
        validator.expect_create_block(
            &block_to_update,
            "## Types of Knowledge Graphs (Updated via WebSocket)",
            Some("test-websocket-page"),
            Some(&original_graph_id),
        );

        // Test Multiple Updates on Same Block (with parent-child and page-to-block edges)
        let _updated_child_id =
            test_multiple_block_updates(&mut ws, &mut validator, &original_graph_id);
        ...
        assert_eq!(
            final_graph_id, original_graph_id,
            "Graph ID changed unexpectedly"
        );
