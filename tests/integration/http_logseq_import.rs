use crate::common::test_harness::{
    assert_phase, read_auth_token, PostShutdown, PreShutdown, TestServer,
};
...
pub fn test_http_logseq_import() {
    ...
    let result = std::panic::catch_unwind(move || {
        ...
        let graph_id = response["graph_id"]
            .as_str()
            .expect("No graph_id in response");
        ...
        let graph_content =
            fs::read_to_string(&graph_path).expect("Failed to read knowledge graph");
        let graph: Value =
            serde_json::from_str(&graph_content).expect("Failed to parse knowledge graph");
        ...
        let ref_content = ref_block["reference_content"]
            .as_str()
            .expect("reference_content field missing");

        assert_eq!(
            ref_content, "## Introduction to Knowledge Graphs",
...
pub fn test_http_import_error_cases() {
    ...
    let result = std::panic::catch_unwind(move || {
        ...
        let response =
            make_import_request(port, "/path/that/does/not/exist", None, Some(&auth_token))
                .expect("Failed to make import request");
        ...
        let response =
            make_import_request(port, temp_file.to_str().unwrap(), None, Some(&auth_token))
                .expect("Failed to make import request");
