        ...
        for i in 0..self.messages.len() {
            for j in (i + 1)..self.messages.len() {
                if self.messages[i] == self.messages[j] {
                    return Err(format!("Duplicate messages found at indices {} and {}", i, j));
                }
            }
        }
...
impl TestValidator {
    ...
    pub fn expect_create_page(&mut self, name: &str, properties: Option<Value>, graph_id: Option<&str>) -> &mut Self {
    ...
    pub fn expect_update_block(&mut self, block_id: &str, new_content: &str, graph_id: Option<&str>) -> &mut Self {
        self.expected_graph_ops.push(ExpectedGraphOp {
            graph_id: graph_id.map(|s| s.to_string()),
    ...
    pub fn expect_graph_created(&mut self, id: Uuid, name: &str) -> &mut Self {
        self.expected_graphs.insert(id, ExpectedGraph {
            name: name.to_string(),
            is_open: false,
        });
    ...
    fn validate_agent(&self) -> Result<(), String> {
        ...
        let history = agent_data["conversation_history"].as_array()
        ...
        if let Some((min, max)) = self.expected_message_count {
            ...
            if count < min {
                return Err(format!("Agent has {} messages, expected at least {}", count, min));
            }
            if let Some(max) = max {
                if count > max {
                    return Err(format!("Agent has {} messages, expected at most {}", count, max));
                }
            }
        }
        ...
        if !self.expected_messages.is_empty() {
            if history.len() != self.expected_messages.len() {
                return Err(format!(
                    ...
                    history.len(), self.expected_messages.len()
                ));
            }
            
            for (index, (actual, expected)) in history.iter().zip(&self.expected_messages).enumerate() {
                let role = actual["role"].as_str()
                ...
                match role {
                    ...
                    "tool" => {
                        if let Some(ref expected_tool) = expected.tool_name {
                            let tool_name = actual["name"].as_str()
                        ...
                        let result_message = actual["result"]["message"].as_str()
                            .ok_or_else(|| format!("Tool message {} missing result.message", index))?;
    ...
    fn validate_registry(&self) -> Result<(), String> {
        ...
        for (id, expected) in &self.expected_graphs {
            ...
            let graph_info = graphs.get(&id_str)
                .ok_or_else(|| format!("Expected graph {} not in registry", id_str))?;
            
            let name = graph_info["name"].as_str()
        ...
        let open_set: HashSet<String> = open_graphs.iter()
    ...
    fn validate_graph_operations(&self) -> Result<(), String> {
        ...
        for op in &self.expected_graph_ops {
            ops_by_graph.entry(op.graph_id.clone()).or_insert_with(Vec::new).push(op);
        }
        ...
        if let Some(ops) = ops_by_graph.get(&None) {
            ...
            if open_graphs.len() == 1 {
                let graph_id = open_graphs[0].as_str()
        ...
        for expected in ops {
            match expected.op_type {
                GraphOpType::CreatePage => {
                    ...
                    let found_node = node_index.iter()
                        .find(|(key, _)| key.to_lowercase() == page_name_lower)
                        .map(|(_, idx)| idx);
                    
                    if let Some(node_idx) = found_node {
                        let node_idx = node_idx.as_u64()
                            .ok_or("Invalid node index")? as usize;
                        ...
                        if node["node_type"].as_str() != Some("Page") {
                            return Err(format!("Expected page '{}' in graph {} but found {}", 
                        ...
                        if let Some(expected_props) = &expected.properties {
                            ...
                            if actual_props != expected_props {
                                return Err(format!("Page '{}' in graph {} properties mismatch", page_name, graph_id));
                            }
                        }
                    } else if !self.deleted_nodes.contains(page_name) {
                        return Err(format!("Expected page '{}' not found in graph {}", page_name, graph_id));
                    }
                },
                GraphOpType::CreateBlock => {
                    let block_id = expected.block_id.as_ref()
                        .ok_or("CreateBlock expectation missing block_id")?;
                    let content = expected.content.as_ref()
                    ...
                    if let Some(node_idx) = node_index.get(block_id) {
                        ...
                        if node["node_type"].as_str() != Some("Block") {
                            return Err(format!("Expected block '{}' in graph {} but found {}", 
                        ...
                        if let Some(page_name) = &expected.page_name {
                            ...
                            if !page_connected {
                                return Err(format!("Block '{}' in graph {} not connected to page '{}'",
                                    block_id, graph_id, page_name));
                            }
                        }
                    } else if !self.deleted_nodes.contains(block_id) {
                        return Err(format!("Expected block '{}' not found in graph {}", block_id, graph_id));
                    }
                },
                GraphOpType::UpdateBlock => {
                    ...
                    if let Some(node_idx) = node_index.get(block_id) {
                        ...
                        if node["content"].as_str() != Some(content) {
                            return Err(format!("Block '{}' in graph {} not updated: expected '{}', got '{:?}'",
                                block_id, graph_id, content, node["content"]));
                        }
                    } else {
                        return Err(format!("Updated block '{}' not found in graph {}", block_id, graph_id));
                    }
                },
                GraphOpType::DeleteBlock => {
                    ...
                    if node_index.contains_key(block_id) {
                        return Err(format!("Block '{}' should be deleted but still exists in graph {}", block_id, graph_id));
                    }
                },
                GraphOpType::DeletePage => {
                    ...
                    if node_index.contains_key(page_name) {
                        return Err(format!("Page '{}' should be deleted but still exists in graph {}", page_name, graph_id));
                    }
                },
            }
        }
