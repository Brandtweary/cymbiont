...
fn verify_all_commands_tested() {
    ...
    if !extra.is_empty() {
        panic!(
            "TESTED_COMMANDS contains commands not in macro: {:?}. Remove them or add to macro.",
            extra
        );
    }
}
    ...
    let result = std::panic::catch_unwind(move || {
        ...
        // Test delete_graph with a secondary graph (keep the imported one)
        {
            ...
            assert_eq!(
                graphs.len(),
                1,
                "Should have only the original graph after deletion"
            );
            assert_eq!(
                graphs[0]["id"].as_str().unwrap(),
                &graph_id,
                "Original graph should remain"
            );
        }
